...
# read in vcf file
gapperi_filtered <- read.vcfR("gapperi.nuc.vcf.gz")
...
# keep only gapperi
popmap <- detailed.popmap[detailed.popmap$nuc_species=="gapperi",]


# make sure popmap is in same order as ids in the vcf
identical(colnames(extract.gt(gapperi_filtered)), popmap$id)
...
vcf2str(vcfR = gapperi_filtered, popmap = popmap, filename = "gapperi.nuc.str")
gapperi_filtered
...
structure <- ggplot(df, aes(x=ancestry, y=id, fill = Group)) +
  ...
  scale_fill_manual(values = c("#A1D99B", "#41AB5D", "#006D2C"))

ggplot2::ggsave(filename = "gapperi.alone.structure.plot.pdf", structure, width = 2, height = 8, path = getwd())
...
gapperi.q.mat <- K3_rep1_structure$q.mat
identical(popmap$id, gapperi.q.mat$id)

popmap$gapperi.q1 <- gapperi.q.mat$Group.1
popmap$gapperi.q2 <- gapperi.q.mat$Group.2
popmap$gapperi.q3 <- gapperi.q.mat$Group.3

write.table(popmap, file = "gapperi.nuc.map.txt", sep = "\t", quote = F, col.names = T, row.names = F)
...
# make sure popmap is in same order as ids in the vcf
identical(colnames(extract.gt(gapperi_filtered)), popmap$id)
# calculate missing data for gapperi-specific dataset
popmap$gap_nuc_missing_data <- (colSums(is.na(gapperi_filtered@gt))/nrow(gapperi_filtered@gt))[-1]

# Q1
summary(lm(gapperi.q1 ~ gap_nuc_missing_data, popmap, subset = (pop == "SEAK")))
summary(lm(gapperi.q1 ~ gap_nuc_missing_data, popmap, subset = (pop == "BC")))
summary(lm(gapperi.q1 ~ gap_nuc_missing_data, popmap, subset = (pop == "NWT")))

q1_r <- c(summary(lm(gapperi.q1 ~ gap_nuc_missing_data, popmap))$r.squared,
          summary(lm(gapperi.q1 ~ pop, popmap))$r.squared,
          summary(lm(gapperi.q1 ~ gap_nuc_missing_data + pop, popmap))$r.squared)

q1_miss <- ggplot(popmap, aes(x=gap_nuc_missing_data, y=gapperi.q1, color=pop)) +
  ...
  ylab(paste("Q1 from structure (C. gapperi only)")) +
  labs(title = "")

ggplot2::ggsave(filename = "gap_missing_q1.png", q1_miss, width = 7, height = 4, path = getwd())

# Q2
summary(lm(gapperi.q2 ~ gap_nuc_missing_data, popmap, subset = (pop == "SEAK")))
summary(lm(gapperi.q2 ~ gap_nuc_missing_data, popmap, subset = (pop == "BC")))
summary(lm(gapperi.q2 ~ gap_nuc_missing_data, popmap, subset = (pop == "NWT")))

q2_r <- c(summary(lm(gapperi.q2 ~ gap_nuc_missing_data, popmap))$r.squared,
          summary(lm(gapperi.q2 ~ pop, popmap))$r.squared,
          summary(lm(gapperi.q2 ~ gap_nuc_missing_data + pop, popmap))$r.squared)

q2_miss <- ggplot(popmap, aes(x=gap_nuc_missing_data, y=gapperi.q2, color=pop)) +
  geom_point(cex = 3, alpha = 1, shape=1) +
  #geom_smooth(method = "lm", se = FALSE) +
  #annotate("text", x=0.48, y=1, label= "R2=6.899e-05") +
  #annotate("text", x=0.48, y=0.95, label= "p=0.9367") +
  xlab(paste("Proportion nuclear missing data")) +
  ylab(paste("Q2 from structure (C. gapperi only)")) +
  labs(title = "")

ggplot2::ggsave(filename = "gap_missing_q2.png", q2_miss, width = 7, height = 4, path = getwd())

#Q3
summary(lm(gapperi.q3 ~ gap_nuc_missing_data, popmap, subset = (pop == "SEAK")))
summary(lm(gapperi.q3 ~ gap_nuc_missing_data, popmap, subset = (pop == "BC")))
summary(lm(gapperi.q3 ~ gap_nuc_missing_data, popmap, subset = (pop == "NWT")))

q3_r <- c(summary(lm(gapperi.q3 ~ gap_nuc_missing_data, popmap))$r.squared,
          summary(lm(gapperi.q3 ~ pop, popmap))$r.squared,
          summary(lm(gapperi.q3 ~ gap_nuc_missing_data + pop, popmap))$r.squared)


q3_miss <- ggplot(popmap, aes(x=gap_nuc_missing_data, y=gapperi.q3, color=pop)) +
  geom_point(cex = 3, alpha = 1, shape=1) +
  #geom_smooth(method = "lm", se = FALSE) +
  #annotate("text", x=0.5, y=1, label= "R2=0.0094") +
  #annotate("text", x=0.5, y=0.95, label= "p=0.3533") +
  xlab(paste("Proportion nuclear missing data")) +
  ylab(paste("Q3 from structure (C. gapperi only)")) +
  labs(title = "")

ggplot2::ggsave(filename = "gap_missing_q3.png", q3_miss, width = 7, height = 4, path = getwd())
...
# get rid of samples with more than 40% missing data
eems.popmap <- eems.popmap[eems.popmap$nuc_missing_data < 0.4,]

# make new vcfR
eems.vcfR <- min_mac(gapperi_filtered[samples=eems.popmap$id], min.mac = 1)
eems.vcfR
...
write.table(eems.popmap[,c("lat", "long")], "datapath.gapperi.coord", col.names = F, row.names = F, quote = F, sep='\t')
...
write.table(diff.mat, "datapath.gapperi.diffs", col.names = F, row.names = F, quote = F, sep='\t')
...
mcmcpath = "rep100.gapperi/"
plotpath = "./rep100.gapperi/rep100_gapperi"
...
load("rep100.gapperi/rep100_gapperi-rdist.RData")
