...
#Make sure to start in correct directory
    cd /kuhpc/work/colella/ben/004.RBV.rad/09.plate.1-5.analyses/01.align.reads/13.mito.both.gstacks/02.alignments

#Get sample name
    name=`basename -a $fastq | cut -f1 -d"."`

#Only do identified gapperi mitos
    alticola=`grep $name ../alticola.mt.txt | wc -l`
    if [[ $alticola -eq 1 ]]
    then
        echo $name >> ../021.aligned.txt
