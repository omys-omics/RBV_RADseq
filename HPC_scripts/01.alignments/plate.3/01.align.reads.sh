...
fastqs=`ls /panfs/pfs.local/work/colella/ben/004.RBV.rad/04.plate.3.analyses/01.process.radtags/04.concatenated.fastqs/*.fq`
...
#Make sure to start in correct directory
    cd /panfs/pfs.local/work/colella/ben/004.RBV.rad/04.plate.3.analyses/02.align.reads/01.alignments/
...
#picard Validate
    picard ValidateSamFile I=$name.raw.bam MODE=SUMMARY > raw.txt
    picard ValidateSamFile I=$name.sorted.bam MODE=SUMMARY > sorted.txt
    echo "validated bam files for $name"
    date
    wait
done
